...
public class MandalsService {
	...
	public String saveMandal(MandalsModel mandalsModel) {
		...
		if (mandalsModel != null) {
			 ...
			 * && !mandalsModel.getName().isEmpty()) && (mandalsModel.getState()
			 * != null && !mandalsModel.getState().isEmpty()) &&
			 * (mandalsModel.getDistrict() != null &&
			 * !mandalsModel.getDistrict().isEmpty())) {
			 */
			if ((mandalsModel.getName() != null && !mandalsModel.getName()
					.isEmpty())
					&& (mandalsModel.getState() != null && !mandalsModel
							.getState().isEmpty())
					&& (mandalsModel.getDistrict() != null && !mandalsModel
							.getDistrict().isEmpty())) {
				...
				// add a call to mandals repository to check if the dta is is
				// already existing
				// or not
	...
	public List<Mandals> getMandalsByStateAndDistrict(String state,
			String district) {
	...
	public String saveCountry(Countries country) {
		...
		return restTemplate.postForObject(
				"http://localhost:9091/countries/save", country, String.class);
	...
	public String getMandalsbaseOnInputparams(MandalsModel mandalsModel) {
		...
		Mandals mandal = mandalsRepository.findMandaslDataByCustomQuery(
				mandalsModel.getState(), mandalsModel.getDistrict(),
				mandalsModel.getName());
		if (mandal != null) {
	...
	public String updateMandals(MandalsModel mandalsModel) {
		...
		if (mandalsModel != null
				&& (mandalsModel.getMid() != 0)
				&& (mandalsModel.getName() != null && !mandalsModel.getName()
						.isEmpty())) {

			Optional<Mandals> mandalFromDB = mandalsRepository
					.findById(mandalsModel.getMid());
			Mandals mandal = null;
	...
	public String deleteMandalByObject(MandalsModel mandalsModel) {
		...
		if (mandalsModel != null && mandalsModel.getMid() != 0) {
			...
			Optional<Mandals> optionalmandal = mandalsRepository
					.findById(mandalsModel.getMid());
